...
export type GetUserTrainData400 = {
  error: string;
  code: string;
};
...
export type getUserTrainDataResponse400 = {
  data: GetUserTrainData400;
  status: 400;
};
 ...
 */
export type useAiResponse200 = {
  data: void;
  status: 200;
};

export type useAiResponseSuccess = useAiResponse200 & {
  headers: Headers;
};
export type useAiResponse = useAiResponseSuccess;

export const getUseAiUrl = () => {
  return `/ai/`;
};

export const useAi = async (options?: RequestInit): Promise<useAiResponse> => {
  return customFetch<useAiResponse>(getUseAiUrl(), {
    ...options,
    method: "POST",
  });
};
